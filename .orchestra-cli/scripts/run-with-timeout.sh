...
CMD="opencode run \"Your Task ID is $TASK_ID. Task: $PROMPT. Signal completion by creating .orchestra/tasks/$TASK_ID.done\" --agent $AGENT_NAME"
...
# Record exit code
echo "$EXIT_CODE" > ".orchestra/tasks/$TASK_ID.exitcode"

# If exit code 124, create timeout sentinel
if [ $EXIT_CODE -eq 124 ]; then
  TIMESTAMP=$(date -Iseconds)
  echo "{\"timeout\": $TIMEOUT, \"timestamp\": \"$TIMESTAMP\"}" > ".orchestra/tasks/$TASK_ID.timeout"
