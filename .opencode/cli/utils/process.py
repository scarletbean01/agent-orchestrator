...
def is_process_alive(pid: int, pid_file: Optional[Path] = None) -> bool:
    ...
    try:
        ...
        print(
            f"WARNING: tasklist command not found (Windows check failed for PID {pid})"
        )
        return False
    except Exception as e:
        # Defensive: log unexpected errors
        print(f"WARNING: Unexpected error checking process {pid}: {e}")
        return False
    ...
    try:
        ...
        if get_os_name() == "windows":
            ...
            try:
                ...
                print(f"DEBUG: Sent graceful termination to PID {pid} (Windows)")
            except subprocess.CalledProcessError:
                # Process might have died already
                if not is_process_alive(pid):
                    return True
            except FileNotFoundError:
                print(f"ERROR: taskkill command not found (cannot kill PID {pid})")
                return False
            ...
            try:
                ...
                print(f"DEBUG: Force killed PID {pid} (Windows)")
                return True
            except subprocess.CalledProcessError:
                ...
                print(
                    f"ERROR: taskkill command not found (cannot force kill PID {pid})"
                )
                return False
            ...
            try:
                ...
                print(f"DEBUG: Sent SIGTERM to PID {pid} (POSIX)")
            except ProcessLookupError:
            ...
            try:
                ...
                print(f"DEBUG: Sent SIGKILL to PID {pid} (POSIX)")
            except ProcessLookupError:
        ...
        # Defensive: log unexpected errors
        print(f"ERROR: Unexpected error killing process {pid}: {e}")
        return False
